...
router.put('/:id', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: `Erro ao atualizar tipo de vegetação: ${err.message}` });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (err.code === 'P2025') { // Erro do Prisma: "Record to delete not found."
        return res.status(404).json({ erro: 'Tipo de vegetação não encontrado para exclusão.' });
    }
    if (err.code === 'P2003') { // Erro do Prisma: "Foreign key constraint failed"
        return res.status(409).json({ erro: 'Não é possível excluir este tipo de vegetação pois está associado a uma ou mais expedições.' });
    }
    return res.status(500).json({ erro: `Erro ao excluir tipo de vegetação: ${err.message}` });
  }
});
